...
bool DistributionInfo::SetupFreshArch()
{
    DWORD exitCode;
    std::wstring cmd = L"/bin/bash /opt/bootstrap/bootstrap.sh";
    HRESULT hr = g_wslApi.WslLaunchInteractive(cmd.c_str(), true, &exitCode);
...
bool DistributionInfo::SetupUser(std::wstring_view userName)
{
    DWORD exitCode;
    std::wstring cmd;

    cmd = L"/usr/bin/passwd ";
    cmd += userName;
    if (FAILED(g_wslApi.WslLaunchInteractive(cmd.c_str(), true, &exitCode)) || exitCode != 0)
        return false;

    cmd = L"mkdir -p /etc/sudoers.d && echo \"%wheel ALL=(ALL) ALL\" > /etc/sudoers.d/99_wheel && chmod 440 /etc/sudoers.d/99_wheel";
    if (FAILED(g_wslApi.WslLaunchInteractive(cmd.c_str(), true, &exitCode)) || exitCode != 0)
        return false;

    cmd = L"echo \"";
    cmd += userName;
    cmd += L"\" > /opt/bootstrap/defuser";
    if (FAILED(g_wslApi.WslLaunchInteractive(cmd.c_str(), true, &exitCode)) || exitCode != 0)
        return false;

    return SetupFreshArch();
}
...
bool DistributionInfo::CreateUser(std::wstring_view userName)
{
    ...
    cmd = L"/usr/bin/useradd -m -G wheel ";
    cmd += userName;
    if (FAILED(g_wslApi.WslLaunchInteractive(cmd.c_str(), true, &exitCode)) || exitCode != 0)
        return false;

    return SetupUser(userName);
}
